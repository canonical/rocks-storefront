...
test("'See all tags' switches to the Tags and channels tab", async ({
  page,
}) => {
  await page.goto("/prometheus");

  await page.getByRole("button", { name: /see all tags/i }).click();
  await expect(
    page.getByText(/tags and channels — coming soon/i),
  ).toBeVisible();
});
