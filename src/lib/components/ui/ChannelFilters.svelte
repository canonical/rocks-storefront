<script lang="ts">
  ...
  // The selects/dropdown render and are interactive, but do not filter the table yet.
...
<div class="channel-filters">
  {#each filters as filter (filter.label)}
    <div class="channel-filters__group">
      ...
      {:else}
        <select id={`filter-${filter.label}`} class="channel-filters__select u-no-margin--bottom">
          {#each filter.options as option (option)}
