...
- `npm test` — full Vitest suite, single run (`test:unit -- --run`)
...
- For each task/slice present in the @plan.md file created, and following the required order for each task:
  ...
  - Iterate between implementation and running tests until the later pass (green).
